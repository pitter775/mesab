            ...
            <div class="card card-plain card_dif">
                <div class="card-header" role="tab" id="headingOne">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="false" aria-controls="collapseOne" class="collapsed">
                        <h5 style="margin: 0; padding:0; font-weight:300"> <i class="fas fa-plus" style="float: left; font-size: 15px; margin-top: 2px; opacity:.5"></i> Relacionar Departamento com Atividade <i class="nc-icon nc-minimal-down"></i></h5>
                        ...
                        <div class="row">
                            <form name="form_informacoes" id="form_informacoes" class="col-md-12">
                                ...
                                <div class="row"> 
                                    <div class="col-md-6">
                                        <div class="md-form">                                                
                                            <select searchable="Procurar..." class="mdb-select colorful-select dropdown-primary md-form" name="dadepartamentos_id" id="dadepartamentos_id" required>
                                                <option value="" disabled selected></option>                                            
                                                    @foreach($departamentos as $key => $value)
                                                        <option value="{{$value->id ?? ''}}">{{$value->depnome ?? ''}}</option>
                                                    @endforeach                                                     
                                            </select>                                                
                                            <label for="dadepartamentos_id" class="active">Selecione um departamento</label>
                                        </div>
                                    </div>  
...
<script>
    ...
    $(document).on('change', '#dadepartamentos_id', function() {atualiza_atividades();});
